...
from app.bot.keyboards import booking_actions_keyboard, booking_confirm_keyboard, cancel_booking_confirm_keyboard, contacts_actions_keyboard, dates_keyboard, main_menu_keyboard, my_bookings_keyboard, services_keyboard, slots_keyboard
from app.bot.service import can_cancel_booking, cancel_booking, create_booking, get_client_booking, get_master_contacts, get_service, get_welcome_text, list_active_services, list_available_dates_for_service, list_available_slots_for_service, list_client_bookings
...
def format_admin_booking_notice(booking, service_name: str, client: Message | CallbackQuery) -> str:
...
async def notify_admins_about_booking(callback: CallbackQuery, booking, service_name: str) -> None:
    for admin_id in get_settings().admin_ids:
        try:
            await callback.bot.send_message(admin_id, format_admin_booking_notice(booking, service_name, callback))
        except Exception:
            continue
...
async def book_date_callback_handler(callback: CallbackQuery) -> None:
    ...
    if service is None:
        ...
        return
    if not slots:
        await safe_edit_text(callback.message, f"{format_service_brief(service)}\n\nНа {target_date.strftime('%d.%m.%Y')} свободных слотов нет.", reply_markup=dates_keyboard(service_id, dates))
        return
    await safe_edit_text(callback.message, f"{format_service_brief(service)}\n\nДата: {target_date.strftime('%d.%m.%Y')}\nВыбери время:", reply_markup=slots_keyboard(service_id, target_date, slots))


@router.callback_query(F.data.startswith("book:slot:"))
async def book_slot_callback_handler(callback: CallbackQuery) -> None:
    _, _, service_id_raw, date_raw, time_raw = callback.data.split(":")
    service_id = int(service_id_raw)
    target_date = date.fromisoformat(date_raw)
    ...
    if service is None:
        await safe_edit_text(callback.message, "Услуга не найдена.", reply_markup=main_menu_keyboard())
        return
    await safe_edit_text(callback.message, f"Подтверди запись\n\n{format_service_brief(service)}\n\nДата: {target_date.strftime('%d.%m.%Y')}\nВремя: {slot.strftime('%H:%M')}", reply_markup=booking_confirm_keyboard(service_id, target_date, slot))
...
async def book_confirm_callback_handler(callback: CallbackQuery) -> None:
    ...
    async with SessionLocal() as session:
        booking = await create_booking(session, telegram_id=callback.from_user.id, username=callback.from_user.username, first_name=callback.from_user.first_name, last_name=callback.from_user.last_name, service_id=service_id, start_at=start_at)
        service = await get_service(session, service_id)
    await safe_answer_callback(callback)
    if booking is None or service is None:
        ...
        return
    await notify_admins_about_booking(callback, booking, service.name)
...
async def my_bookings_callback_handler(callback: CallbackQuery) -> None:
    ...
    if not bookings:
        ...
        return
    items = [(booking.id, f"{booking.start_at.strftime('%d.%m %H:%M')} • {booking.service.name} • {booking_status_label(booking.status)}") for booking in bookings]
